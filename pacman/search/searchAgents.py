...
def cornersHeuristic(state, problem):
    ...
    if len(unvisited) == 0:
        return 0
    combinations = tuple(itertools.permutations(unvisited))
    heuristic = float("inf")
    for combination in combinations:
        if manhattan(state[0], combination) < heuristic:
            heuristic = manhattan(state[0], combination)
    return heuristic

def manhattan(position, combination, info={}):
    "The Manhattan distance heuristic for a PositionSearchProblem"
    xy1 = position
    xy2 = combination[0]
    ...
    for i in range(1,len(combination)):
        xy1 = combination[i-1]
        xy2 = combination[i]
        dis = dis + abs(xy1[0] - xy2[0]) + abs(xy1[1] - xy2[1])
    return dis
...
def foodHeuristic(state, problem):
    ...
    for food in foods:
        heuristic = heuristic + abs(position[0] - food[0]) + abs(position[1] - food[1])
    return heuristic
