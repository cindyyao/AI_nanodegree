...
function drawFeaturePoints(canvas, img, face) {
  ...
  // See: https://developer.mozilla.org/en-US/docs/Web/API/CanvasRenderingCohttps://developer.mozilla.org/en-US/docs/Web/API/CanvasRenderingContext2D#Fill_and_stroke_stylesntext2D#Fill_and_stroke_styles
  // <your code here>
...
function drawEmoji(canvas, img, face) {
  // Obtain a 2D context object to draw on the canvas
  var ctx = canvas.getContext('2d');
  var dEmoji = face.emojis.dominantEmoji;
  ...
  for (var id in face.featurePoints) {
    ...
    if (featurePoint.y < ymin) {
      ymin = featurePoint.y;
    }
  }
  ctx.fillText(dEmoji, xmax + 50, ymin - 50);
}
...
function mimic(face) {
  if (toUnicode(face.emojis.dominantEmoji) == newEmoji) {
    ScoreCorrect++;
    setScore(ScoreCorrect, ScoreTotal);
    if (timer) {
      clearInterval(timer);
